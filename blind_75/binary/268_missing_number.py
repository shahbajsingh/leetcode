...
Given an array 'nums' containing 'n' distinct numbers in the range [0,n], return only 
the number in the range that is missing from the array.
...
class Solution:
    def missingNumber(self, nums: List[int]) -> int:
        n = len(nums) + 1 # of [0,n]
        arr = sum(range(n)) # nth triangular number
        
        return arr - sum(nums) # return difference
        
sol = Solution()
sol.missingNumber([0,1,2,3,4,5,6,7,9])

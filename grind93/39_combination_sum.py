...
class Solution:
    def combinationSum(self, candidates: List[int], target: int) -> List[List[int]]:
        ...
        def backtrack(start, combination, total):
            ...
            # explore combinations from curr index to allow unlimited use
            for i in range(start, len(candidates)):
                combination.append(candidates[i])
                backtrack(i, combination, total + candidates[i]) # recur with updated total
                combination.pop() # undo choice
                
        backtrack(0, [], 0)
        return result

# Time Complexity: O(2^t) where 't' is target value
# Space Complexity: O(t / min(candidates)) due to max depth of recursion tree
#                   where minimum element may be used repeatedly